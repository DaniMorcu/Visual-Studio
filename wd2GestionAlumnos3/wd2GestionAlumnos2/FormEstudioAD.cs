...
namespace EstudioAccesoDatos
{
    public partial class FormEstudioAD : Form
    {
        ...
        private bool HayErrorEnDatos()
        {
            ...
            String errorNombre = String.Empty;
            String errorTelefonosVacios = String.Empty;
            String errorGrupo = String.Empty;
            String errorComienzoTelf = String.Empty;
            String errorComienzoMovil = String.Empty;
            String errorDni = String.Empty;
            String errorMail = String.Empty;
            String patronDni = String.Empty;
            String patronEmail = String.Empty;
            String dniOk = String.Empty;
            String telefonoOK = String.Empty;
            String movilOK = String.Empty;
            String nombreOk = String.Empty;
            String emailOk = String.Empty;

            // Vamos a comprobar que el nombre esta separado por una ", "
            nombreOk = txbApellNom.Text.ToString();
            if (!nombreOk.Contains(", "))
            {
                errorNombre = "Nombre incorrecto\n";
                hayErrores = true;
            }

            // Solo puede estar uno de los dos teléfonos vacios
            if(txbTelefono.Text.ToString() == String.Empty && txbMovil.Text.ToString() == String.Empty)
            {
                errorTelefonosVacios = "Rellene al menos un telefono de contacto\n";
                hayErrores = true;
            }

            // DNI con 8 digitos y una letra
            patronDni = @"(\d{8})([-]?)([A-Z]{1})";
            dniOk = txbDni.Text.ToString();
            if (!Regex.IsMatch(dniOk, patronDni))
            {
                errorDni = "El formato del DNI es incorrecto (12345678-A)\n";
                hayErrores = true;
            }

            // Se tiene que haber seleccionado un Grupo
            if (cbGruposDetalle.SelectedIndex == -1)
            {
                errorGrupo = "Seleccione un grupo para el alumno\n";
                hayErrores = true;
            }

            // Comprobar telefono empieza por 955 o 954
            telefonoOK = txbTelefono.Text.ToString();
            
            if(telefonoOK.Trim().Length != 0)
            {
                if (!(telefonoOK.Substring(0, 3).Equals("955") || telefonoOK.Substring(0, 3).Equals("954")))
                {
                    errorComienzoTelf = "El telefono fijo tiene que comenzar por 955 o 954\n";
                    hayErrores = true;
                }
            }
         

            // Comprobar numero inicial del móvil
            movilOK = txbTelefono.Text.ToString();

            if(movilOK.Trim().Length != 0)
            {
                if (!(movilOK.Substring(0, 1).Equals("6") || movilOK.Substring(0, 1).Equals("7")))
                {
                    errorComienzoMovil = "El numero móvil debe comenzar por 6 o 7\n";
                    hayErrores = true;
                }
            }
            ...
            // Comprobar el email
            patronEmail = @"\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*";
            emailOk = txbMail.Text.ToString();
            if (!Regex.IsMatch(dniOk, patronDni))
            {
                errorMail = "El formato del email es incorrecto (ejemplo: alumno@colegio.com)\n";
                hayErrores = true;
            }

            if (hayErrores)
            {
                MessageBox.Show("Errores localizados: \n" + errorNombre +
                    errorTelefonosVacios +
                    errorDni +
                    errorGrupo +
                    errorComienzoTelf +
                    errorComienzoMovil +
